# The Manor in the Mirrorline - Safety

## Egress

- Normal exit: main trailer door remains visible and unobstructed.
- Emergency exit: must be visible or explicitly identified in briefing once trailer model is known.
- Staff override: operator can stop game, turn up lights, release final state, and direct exit.
- Player-visible reassurance: intro states players are never actually locked in and can leave any time.
...
| spooky tone | anxiety or startle | no actor jump scares, clear exit, bounded sound | final audio intensity |
...
- Monitoring: camera/audio on door, accusation cabinet, object tray, and station areas.
- Emergency stop: turn up lights, stop audio, disable powered props, direct exit.
- Communication: operator can speak in fiction as the house ledger or estate steward.
- Extraction script: "The house is paused. Please step to the visible exit now."

## Safety gate

| Requirement | Pass? | Evidence | Required revision |
|---|---|---|---|
| Normal exit remains usable | TBD | floorplan draft | Verify with actual trailer and cabinet footprint. |
